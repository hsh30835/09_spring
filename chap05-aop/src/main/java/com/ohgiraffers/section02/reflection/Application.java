...
public class Application {
    /* 리플렉션이란?
    * 컴파일된 자바 코드에서 역으로 클래슬르 불러 메소드 및 필드 정보를 구해오는 방법으로 반사, 투영이라는 의미를 가진다.
    * 리플렉션은 jvm에서 실행되는 애플리케이션의 런타임 동작을 검사하거나 수정할 수 있는 기능이 필요한 경우 사용한다.
    * 스프링프레임워크, 마이바티스, 하이버네이트(jpa), Jackson 등의 라이브러리에서 사용한다.
    * reflection을 사요해서 스프링에서는 런타임시에 개발자가 등록한 빈을 어플리케이션 내에서 사용할 수 있게 하는 것이다.
    *
    * reflection은 강력한 도구이지만 무분별하게 사용해서는 안된다.
    * 1. 오버헤드 발생 : 성능의 저하를 발생할 수 있기 때문에 성능에 민감한 어플리케이션에서는 사용하지 않는다
    ...
    * */
    public static void main(String[] args){
        ...
        // 원시 자료형을 사용하면 컴파일 에러 발생 원시자료형은 클래스가 아님 그래서 에러남
//        double d = 1.0;
//        Class class8 = d.getClass();
        ...
        /*
        * 클래의 메타 정보를 이용하여 여러가지 정보를 반환받는 메소드를 제공한다.
        * 상속된 부모 클래스를 반환한다.
        * */
        ...
        try {
            Account acc = (Account)constructors[1].newInstance("20", "110-223-123456", "1234", 100);
